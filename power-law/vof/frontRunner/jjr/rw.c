...
// #define mu(muTemp, mu2, f)  (1./(clamp(f,0.,1.)*(1./muTemp - 1./mu2) + 1./mu2))
...
#define FR 1.756 // Froude number for the expected steady-state solution, but this dimensionless parameter is not used in the codes here.
#define MUDRHO 2130.0 //density ratio, water to air
// #define MURATIO 8.9e-4/17.4e-6 //dynamic viscosity ratio, water to air
#define POWERLAWINDEX 0.30
#define MUMUD 150.0

#define AIRRHO 1.12
#define AIRMU (0.001/45.0)
...
#define MAXTIME 72.0 // Maximum runtime.
#define TOUTPUT 0.20
...
// square domain size
#define xextent_ (819.2*NORMALDEPTH)
#define topExtent (NORMALDEPTH*3.90)
 ...
 * **/
  ...
  */
  ...
  // uf.n[left]  = dirichlet(inletDistVel(y, t));
...
event init (i=0)
{
    ...
    // unrefine(y>topExtent*1.20 && level>MAXLEVEL-6);
...
int refRegion(double x, double y, double z){
  return (y<=3.25*NORMALDEPTH ? MAXLEVEL : (MAXLEVEL-2));
}
...
//   adapt_wavelet ((scalar *){f, u.x, u.y, KAPPA, omega}, (double[]){fErr, VelErr, VelErr, KAPPAErr, OmegaErr}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  ...
  refine(y<=xextent_/pow(2,MAXLEVEL)*8.10 && level<MAXLEVEL);
