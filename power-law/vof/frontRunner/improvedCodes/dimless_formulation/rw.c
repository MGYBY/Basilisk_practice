...
#define NORMALVELMAX (1.0/(1.0-POWERLAWINDEX/(2.0*POWERLAWINDEX+1.0)*(1.0-BPARAM)))

// inclination angle of the channel. \sin\theta and \cos\theta
#define CHANNELTAN 0.060
...
#define FR 0.80 // Froude number for the expected steady-state solution, but this dimensionless parameter is not used in the codes here.
#define RE (1.0/(CHANNELTAN*(pow((POWERLAWINDEX/(POWERLAWINDEX+1.0)*pow((1.0/(FR*FR)), (1.0/POWERLAWINDEX))*pow((1.0-BPARAM), ((POWERLAWINDEX+1.0)/POWERLAWINDEX))*(1.0-(POWERLAWINDEX)/(2.0*POWERLAWINDEX+1.0)*(1.0-BPARAM))), POWERLAWINDEX))))
#define MUMUD (1.0/RE)

#define MUMAXVAL 60.0

#define AIRRHO (MUDRHO/rhoR) // 1.12
// #define AIRMU (MUMUD/50.0)
#define AIRMU ((1.0/RE+CHANNELTAN*BPARAM/(FR*FR))/muR)
...
#define DISTWAVELENGTH (2.00/CHANNELTAN) //dummy here

// square domain size
// #define xextent_ (4.716*NORMALDEPTH/CHANNELTAN)
#define xextent_ (1150.0*NORMALDEPTH)
#define topExtent (NORMALDEPTH*6.75)
#define initTransitCells 30

// #define KAPPAErr (1e-3)
#define OmegaErr (NORMALVEL/NORMALDEPTH/2.999)
#define fErr (1e-7)
...
double distVel(double xCoord, double yCoord)
{
  ...
  double distDepth = (xCoord>=DISTWAVELENGTH/2.0 && xCoord<=DISTWAVELENGTH) ? (NORMALDEPTH*(1.0+DISTAMP*sin(2.0*pi*(xCoord-DISTWAVELENGTH/2.0)/DISTWAVELENGTH))) : NORMALDEPTH;
...
//       refine(y<(7.0*NORMALDEPTH) && level < (MAXLEVEL-3));
      ...
      foreach() {
        ...
        p[] = (y<=distSurf(x)) ? (distSurf(x)-y)*MUDRHO : (-1.0)*AIRRHO*(y-distSurf(x));
...
//   MPI_Allreduce (MPI_IN_PLACE, b, 3*n, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  ...
  foreach()
  {
    ypos[] = volDroplet[]>(900.1*sq(xextent_/pow(2, MAXLEVEL))) ? ypos[] : 0.0;
  }
...
//   adapt_wavelet ((scalar *){f, u.x, u.y, KAPPA, omega}, (double[]){fErr, VelErr, VelErr, KAPPAErr, OmegaErr}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  ...
  refine(y<=(8.590*xextent_/pow(2, MAXLEVEL)) && level<MAXLEVEL);
