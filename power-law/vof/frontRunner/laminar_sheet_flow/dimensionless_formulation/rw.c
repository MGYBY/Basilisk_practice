...
#define FR 0.570 // Froude number for the expected steady-state solution, but this dimensionless parameter is not used in the codes here.
...
#define xextent_ (1300.0*NORMALDEPTH)
#define topExtent (NORMALDEPTH*4.60)
#define initTransitCells 30

// #define KAPPAErr (1e-3)
#define OmegaErr (NORMALVEL/NORMALDEPTH/24.0)
#define fErr (1e-7)
#define VelFluidErr (NORMALVEL/45.0)
...
//       refine(y<(7.0*NORMALDEPTH) && level < (MAXLEVEL-3));
      ...
      foreach() {
        ...
        u.x[] = (y<=topExtent*1.0) ? y<=distSurf(x) ? 3.0*(y-0.5*y*y) : (y<=(distSurf(x)+initTransitCells*xextent_/pow(2, MAXLEVEL))) ? 3.0*(distSurf(x)-0.5*distSurf(x)*distSurf(x))*((distSurf(x)+initTransitCells*xextent_/pow(2, MAXLEVEL)-y)/(initTransitCells*xextent_/pow(2, MAXLEVEL))) : 0.0 : 0.0;
        // u.x[] = 0.0;
        u.y[] = 0.0;
...
//   adapt_wavelet ((scalar *){f, u.x, u.y, KAPPA, omega}, (double[]){fErr, VelErr, VelErr, KAPPAErr, OmegaErr}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  ...
  refine(y<=(9.510*xextent_/pow(2, MAXLEVEL)) && level<MAXLEVEL);
