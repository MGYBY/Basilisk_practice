...
void embed_vof_force_3D (scalar p, vector u, face vector mu, scalar color, coord * Fp, coord * Fmu)
{
  // fixme: this could be simplified considerably if reduction worked on vectors
  double Fp_x = 0., Fp_y = 0., Fp_z = 0., Fmu_x = 0., Fmu_y = 0., Fmu_z = 0.;
  double colorThreshold = 1e-5;
  foreach (reduction(+:Fp_x) reduction(+:Fp_y) reduction(+:Fp_z)
	   reduction(+:Fmu_x) reduction(+:Fmu_y) reduction(+:Fmu_z)) {
    if (cs[] > 0. && cs[] < 1. && color[] > colorThreshold ) {

      coord n, b;
      double area = embed_geometry (point, &b, &n);
      area *= pow (Delta, dimension - 1);

      double Fn = area*color[]*embed_interpolate_3D (point, p, b);
      foreach_dimension()
	Fp_x += Fn*n.x;
...
#if dimension == 2
	foreach_dimension()
	  Fmu_x -= area*color[]*mua*(dudn.x*(sq (n.x) + 1.) +
			     dudn.y*n.x*n.y);
#else // dimension == 3
	foreach_dimension()
	  Fmu_x -= area*color[]*mua*(dudn.x*(sq (n.x) + 1.) +
