...
void p_shape (scalar c, face vector f)
{
  vertex scalar phi[];
  foreach_vertex()
    phi[] = max(max(leftXCoord-x, x-rightXCoord), z-topZCoord);
  fractions (phi, c, f);
...
int refRegion(double x, double y, double z){
  return (y<=15.0*NORMALDEPTH && x<=5.50*NORMALDEPTH ? MAXLEVEL : (MAXLEVEL-3));
}
...
//   adapt_wavelet ((scalar *){f, u.x, u.y, KAPPA, omega}, (double[]){fErr, VelErr, VelErr, KAPPAErr, OmegaErr}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  ...
  adapt_wavelet_limited ((scalar *){f, omega, KAPPA}, (double[]){fErr, OmegaErr, KErr}, refRegion, minlevel = MINLEVEL);
  refine(y<=xextent_/pow(2,MAXLEVEL)*9.10 && x<=5.50*NORMALDEPTH && level<MAXLEVEL);
  p_shape (cs, fs);
