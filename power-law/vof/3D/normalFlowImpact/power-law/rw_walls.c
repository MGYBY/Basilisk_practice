...
#define NORMALDEPTH 0.0022311068
#define NORMALVEL 0.177371713
...
// square domain size
#define xextent_ (11.50*NORMALDEPTH)
#define zextent_ (4.5*NORMALDEPTH)
#define topExtent (5.0*NORMALDEPTH)
// domain length excluding outlet
#define mainDomainLength (10.0*NORMALDEPTH)
...
#define leftXCoord (6.0*NORMALDEPTH)
// #define rightXCoord (centXCoord+obsWidth*0.50)
#define rightXCoord (leftXCoord+NORMALDEPTH)
#define topZCoord (0.0+obsWidth*0.50)
#define bottomZCoord (0.0)

#define KAPPAErr (1e-3)
#define OmegaErr (5.99)
#define fErr (5.0e-5)
...
void p_shape (scalar c, face vector f)
{
  vertex scalar phi[];
  foreach_vertex()
    phi[] = min(min(max(max(leftXCoord-x, x-rightXCoord), z-topZCoord), zextent_-z), topExtent-y);
  fractions (phi, c, f);
//   fractions_cleanup (c, f, smin = 1.e-14, cmin = 1.e-14);
    fractions_cleanup (c, f, smin = 1.e-14);
}
...
event init (i=0)
{
    ...
    do {
      ...
      // vorticity (u, omega);
      refine(y>=(NORMALDEPTH-2.50*(xextent_/pow(2,MAXLEVEL))) && y<=(NORMALDEPTH+2.50*(xextent_/pow(2,MAXLEVEL))) && level<MAXLEVEL);
      // curvature(f, KAPPA);
      ...
      foreach() {
        ...
        u.x[] = (y<=topExtent*1.10) ? (y<=NORMALDEPTH ? cs[]*normalFlowVel(y) : cs[]*normalFlowVel(NORMALDEPTH)) : 0.0;
        // u.x[] = 0.0;
        // u.x[] = 0.0;
        u.y[] = 0.0;
        u.z[] = 0.0;
    ...
    while (adapt_wavelet ((scalar *){f, u.x}, (double[]){(fErr/4.0), VelErr}, maxlevel = MAXLEVEL, minlevel = MINLEVEL).nf);
    unrefine(y>topExtent*1.10 && z>zextent_*1.10 && level>MAXLEVEL-4);
    // refine(y<=xextent_/pow(2,MAXLEVEL)*5.10 && level<MAXLEVEL);
//     jdx = 0;
//     do {
//       jdx += 1;
...
int refRegion(double x, double y, double z){
  return (y<=(1.1*topExtent) && x<=mainDomainLength && z<=(1.1*zextent_)? MAXLEVEL : (MAXLEVEL-4));
}
...
event calcForce(i=2; i++) {
  FILE *fp1 = fopen("forceCalc.txt", "a+");
  coord Fp, Fmu;
  embed_vof_force_3D (p, u, mu, f, &Fp, &Fmu); 
  fprintf(fp1, "%g %g %g %g \n", t, Fp.x, Fmu.x, (Fp.x+Fmu.x));
  fclose(fp1);
  if (i%5 == 1) {
...
//   adapt_wavelet ((scalar *){f, u.x, u.y, KAPPA, omega}, (double[]){fErr, VelErr, VelErr, KAPPAErr, OmegaErr}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  ...
  adapt_wavelet_limited ((scalar *){f, cs, omega, KAPPA}, (double[]){fErr, 0.02, OmegaErr, KErr}, refRegion, minlevel = MINLEVEL);
  refine(y<=xextent_/pow(2,MAXLEVEL)*9.10 && x<=mainDomainLength && z<=4.5*NORMALDEPTH && level<MAXLEVEL);
  p_shape (cs, fs);
...
event movies (i += 4) {
  view (quat = {-0.160, -0.250, -0.058, 0.953},
        fov = 30, near = 0.01, far = 1000,
        tx = -0.574, ty = -0.303, tz = -3.170,
        width = 1620, height = 874);
  box ();
  squares (color = "u.x", linear = true, n = {0,0,1});
  cells (n = {0,0,1});
//   box ();
  draw_vof (c = "f", fc = {0.7843137254901961,0,0.7843137254901961}, lc = {0.7843137254901961,0,0.803921568627451});
  draw_vof (c = "cs");
  save ("interfaceMovie.mp4");
}
