...
double powerLawIndex = 1.0, muRef = 0.001, mumax = 1.0;
...
numFilter = 1;
  ...
  while (numFilter>0)
  {
    foreach()
      f[] = (4.*sf[] +
	    2.*(sf[0,1] + sf[0,-1] + sf[1,0] + sf[-1,0]) +
	    sf[-1,-1] + sf[1,-1] + sf[1,1] + sf[-1,1])/16.;
    swap (scalar, f, sf);
    numFilter--;
  }
  ...
  foreach()
    ...
    {
      f[] = (8.*sf[] +
	    4.*(sf[-1] + sf[1] + sf[0,1] + sf[0,-1] + sf[0,0,1] + sf[0,0,-1]) +
	    2.*(sf[-1,1] + sf[-1,0,1] + sf[-1,0,-1] + sf[-1,-1] +
		sf[0,1,1] + sf[0,1,-1] + sf[0,-1,1] + sf[0,-1,-1] +
		sf[1,1] + sf[1,0,1] + sf[1,-1] + sf[1,0,-1]) +
	    sf[1,-1,1] + sf[-1,1,1] + sf[-1,1,-1] + sf[1,1,1] +
	    sf[1,1,-1] + sf[-1,-1,-1] + sf[1,-1,-1] + sf[-1,-1,1])/64.;
      swap (scalar, f, sf);
      numFilter--;
    }
#endif
  ...
  foreach_face() {
    ...
    if (mu1 || mu2) {
      face vector muv = mu;
        ...
        double D22 = ((u.y[0,1]-u.y[])+(u.y[-1,1]-u.y[-1,0]))/2.0;
        // double D12 = 0.5*(((u.x[0,1]-u.x[0,-1])+(u.x[-1,1]-u.x[-1,-1]))/4.0 + (u.y[] - u.y[-1,0]));
        // double D12 = 0.5*(((u.x[1,1]-u.x[1,-1])+(u.x[0,1]-u.x[0,-1]))/4.0 + (u.y[] - u.y[-1,0]+u.y[0,1] - u.y[-1,1])/2.0);
        double D12 = 0.5*((u.x[0,1]-u.x[0,-1])/2.0 + (u.y[] - u.y[-1,0]+u.y[0,1] - u.y[-1,1])/2.0);
        // double D2 = sqrt(sq(D11)+sq(D22)+2.0*sq(D12))/(Delta);
        // double D11 = (u.x[1,0] - u.x[]);
        // double D22 = (u.y[0,1]-u.y[]);
        // double D12 = 0.5*(((u.y[1,0]+u.y[1,1])/2.0-(u.y[-1,0]+u.y[-1,1])/2.0)/2.0+((u.x[0,1]+u.x[1,1])/2.0-(u.x[0,-1]+u.x[1,-1])/2.0)/2.0);
        // double D11 = (u.x[1,0] - u.x[-1,0])/2.0;
        // double D22 = (u.y[0,1,0] - u.y[0,-1,0])/2.0;
        // double D12 = 0.5*(u.x[0,1] - u.x[0,-1] + u.y[1,0] - u.y[-1,0])/2.;
        double D2 = sqrt(sq(D11)+sq(D22)+2.0*sq(D12))/(Delta);
      #else // dimension == 3
        // double D2 = 0.;
        ...
        // double D2 = sq(dxx) + sq(dyy) +sq(dzz) +2.*sq(dxy) +2.*sq(dxz) +2.*sq(dyz);
