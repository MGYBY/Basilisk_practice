...
#define FR 0.75 // Froude number for the expected steady-state solution, but this dimensionless parameter is not used in the codes here.
...
#define xextent_ (8.0*NORMALDEPTH/CHANNELTAN)
#define topExtent (NORMALDEPTH*3.99)
#define initTransitCells 30

// #define KAPPAErr (1e-3)
#define OmegaErr (NORMALVEL/NORMALDEPTH/2.75)
#define fErr (1e-7)
#define VelFluidErr (NORMALVEL/40.20)
#define VelAirErr (NORMALVEL/12.050)
#define KErr (1e-3)
...
//   adapt_wavelet ((scalar *){f, u.x, u.y, KAPPA, omega}, (double[]){fErr, VelErr, VelErr, KAPPAErr, OmegaErr}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  ...
  refine(y<=(9.590*xextent_/pow(2, MAXLEVEL)) && level<MAXLEVEL);
