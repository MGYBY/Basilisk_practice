...
#define MAXTIME 100.0 // Maximum runtime.
#define TOUTPUT 0.25
...
// square domain size
#define xextent_ (63.05*NORMALDEPTH)
...
//   G.x = (CHANNELSLOPE)*g_;
  ...
  const face vector gravity[] = {(CHANNELSLOPE)*GRAV, (-CHANNELCOS)*GRAV};
//   const face vector gravity[] = {(CHANNELSLOPE)*GRAV*f, (-CHANNELCOS)*GRAV*f};
  a = gravity;

//   solid (cs, fs, (NORMALDEPTH*4.01) - y );
...
event init (i=0)
{
    ...
    while (adapt_wavelet ({f,u}, (double[]){femax,uemax, uemax}, MAXLEVEL, MINLEVEL).nf);
