...
#define hemax (normalDepth/250.0)
#define uemax (normalVelocity/500.0)
#define hmaxemax (hemax/2.05)
...
event init(i = 0)
{
  ...
  refine(y<=(distLength*2.0) && x<=(disPeriod*normalVelocity*2.5) && level<MAXMAXLEVEL);
...
event adapt(i++)
{
  astats s = adapt_wavelet({h, hmax, hGradMag}, (double[]){hemax, hmaxemax, 0.02}, maxlevel = MAXMAXLEVEL, minlevel = MINLEVEL);
