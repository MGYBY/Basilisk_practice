...
#define hemax (normalDepth/200.0)
#define uemax (normalVelocity/500.0)
#define hmaxemax (hemax/2.00)
...
event init(i = 0)
{
  ...
  refine(fabs(LyCenter-y)<=(distLength*1.51/2.0) && x<=(disPeriod*normalVelocity*2.5) && level<MAXMAXLEVEL);
...
//   mask(x >= leftCoord ? (x <= rightCoord ? (y >= bottomCoord ? (y <= topCoord ? block : none) : none) : none) : none);
  ...
  h[left] = dirichlet((t<=disPeriod/2.0 && fabs(LyCenter-y)<=(distLength/2.0)) ? normalDepth + disMag * normalDepth * sin(2. * pi * t / disPeriod) : normalDepth);
  u.n[left] = dirichlet((t<=disPeriod/2.0 && fabs(LyCenter-y)<=(distLength/2.0)) ? frn*sqrt(gp*(normalDepth + disMag * normalDepth * sin(2. * pi * t / disPeriod))) : normalVelocity);
...
//   }
  ...
  output_gfs(fp2, translate = true, list = {h, u.x, u.y, hGradMag});
...
int refRegion(double x, double y, double z){
  return ((fabs(LyCenter-y)<=20*distLength) ? MAXMAXLEVEL: MAXLEVEL);
}

// adaptivity
event adapt(i++)
{
  adapt_wavelet_limited({h, hmax, hGradMag}, (double[]){hemax, hmaxemax, 0.04999}, refRegion, minlevel = MINLEVEL);
  //fprintf(stderr, "# refined %d cells, coarsened %d cells\n", s.nf, s.nc);

  refine(t<=disPeriod*2.0 && fabs(LyCenter-y)<=(distLength*1.250/2.0) && x<=(disPeriod*normalVelocity*2.5) && level<MAXMAXLEVEL);
