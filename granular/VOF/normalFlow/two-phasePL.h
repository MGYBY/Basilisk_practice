...
double froude = 0.0, mus = 0.0, deltaMu = 0.0, dg = 0.0, phiVF = 0.0, I0 = 0.0, mumax = 1.0;
...
//         double temp = (1.0/froude/froude)*(mus*p[]/D2+deltaMu*p[]/((1./froude*I0*pow(phiVF*p[],0.50)/(dg))+D2));

        double In = D2*dg/sqrt(fabs(phiVF*p[]));
        double muI = mus + deltaMu*In/(I0 + In);
        double etamin = 1.0*sqrt(dg*dg*dg);
        muTemp = max(muI*p[]/D2, etamin);
//         muTemp = muI*p[]/D2;
        muTemp = min(muTemp, mumax);

//         double temp = (mus*p[]/D2+deltaMu*p[]/(I0/(dg/pow(fabs(phiVF*p[]),0.50))+D2));
