...
#define xextent_ (4.00/CHANNELTAN)

// #define KAPPAErr (1e-3)
#define OmegaErr (NORMALVEL/NORMALDEPTH/19.90)
#define fErr (1e-7)
#define VelFluidErr (NORMALVEL/40.50)
#define VelAirErr (NORMALVEL/20.50)
#define KErr (1e-3)
...
{
  return (NORMALDEPTH*(1.0+DISTAMP*sin(2.0*pi*xCoord/(xextent_/1.0))));
}

double distVel(double xCoord, double yCoord)
{
  double distDepth = NORMALDEPTH*(1.0+DISTAMP*sin(2.0*pi*xCoord/xextent_));
  double hR1 = (NORMALDEPTH*(1+hR)-distDepth)/distDepth;
  double normalFlowFR = (muR*(hR*(4.0+3.0*hR)+muR)+hR*hR*rhoR*(hR*hR+muR*(3.0+4.0*hR)))/((1+hR)*muR*((4*hR+muR)+3.0*hR*hR*rhoR));
  double distFlowFR = (muR*(hR1*(4.0+3.0*hR1)+muR)+hR1*hR1*rhoR*(hR1*hR1+muR*(3.0+4.0*hR1)))/((1+hR1)*muR*((4*hR1+muR)+3.0*hR1*hR1*rhoR));
  double distVelFac = (normalFlowFR)/(distFlowFR);

  // we keep the discharge constant
  if (yCoord<=distDepth)
    return distVelFac*((6*(yCoord/distDepth)*(rhoR + hR1*muR*(hR1 + 2*rhoR) - (1 + hR1*muR)*rhoR*(yCoord/distDepth)))/(rhoR + hR1*muR*(3*hR1 + 4*rhoR)));
  else if (yCoord>distDepth && yCoord<=topExtent)
    return distVelFac*((6*((NORMALDEPTH*(1+hR)-yCoord)/(NORMALDEPTH*(1+hR)-distDepth))*(rhoR - hR1*(-2 + hR1*muR*(-1 + ((NORMALDEPTH*(1+hR)-yCoord)/(NORMALDEPTH*(1+hR)-distDepth))) + ((NORMALDEPTH*(1+hR)-yCoord)/(NORMALDEPTH*(1+hR)-distDepth)))))/(hR1*(4 + hR1*muR) + 3*rhoR));
  else
    return (0.0);
}
...
//       refine(y<(7.0*NORMALDEPTH) && level < (MAXLEVEL-3));
      ...
      refine(y>(topExtent-10.0*NORMALDEPTH) && level < (MAXLEVEL-1));
      refine(y>(topExtent-2.490*NORMALDEPTH) && level < MAXLEVEL);
      ...
      foreach() {
        ...
        u.x[] = (y<=topExtent*1.0) ? distVel(x, y) : 0.0;
        // u.x[] = 0.0;
        u.y[] = 0.0;
...
event outputGfsFiles (t += TOUTPUT) {
    ...
    sprintf(nameBSSBottom, "bssBottom-%g.dat", t);
    sprintf(nameBSSTop, "bssTop-%g.dat", t);
    FILE *fp1 = fopen(name, "w");
    FILE *fp2 = fopen(nameBSSBottom, "a+");
    FILE *fp3 = fopen(nameBSSTop, "a+");
    ...
    bedYTop = topExtent - xextent_/pow(2,MAXLEVEL)/2.0;
    for (int l = 1; l <= pow(2,MAXLEVEL); l++)
    {
      ...
      bedStress = bedVel/bedY;
      fprintf(fp2, "%g %g %g\n", bedX, bedVel, bedStress);

//       bedX = xextent_/pow(2,MAXLEVEL)*l-0.5*xextent_/pow(2,MAXLEVEL);
      bedVel = interpolate(u.x, bedX, bedYTop, 0.0);
      bedStress = bedVel/bedY;
      fprintf(fp3, "%g %g %g\n", bedX, bedVel, bedStress);
    }
    fclose (fp2);
...
//   adapt_wavelet ((scalar *){f, u.x, u.y, KAPPA, omega}, (double[]){fErr, VelErr, VelErr, KAPPAErr, OmegaErr}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  ...
  refine(y<=(6.510*xextent_/pow(2, MAXLEVEL)) && level<MAXLEVEL);
  refine((y>=(topExtent-3.470*xextent_/pow(2, MAXLEVEL)) && y<=topExtent) && level<MAXLEVEL);
