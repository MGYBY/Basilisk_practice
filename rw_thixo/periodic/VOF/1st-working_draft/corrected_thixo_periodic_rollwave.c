...
#endif
#ifndef THIXO_MU_MAX
#define THIXO_MU_MAX (2.5*(1.0/THIXO_SO)) // roughly estimated from Balmforth as 1/\epsilon
#endif
#ifndef THIXO_SIGMA
...
event output_gfs_files (t = OUTPUT_DT; t += OUTPUT_DT)
{
  char name[128];
...
event output_interface (t = OUTPUT_DT; t += OUTPUT_DT)
{
  char localname[80];
...
event output_centerline_profile (t = OUTPUT_DT; t += OUTPUT_DT)
{
  char name[128];
...
event adapt (i++) {
  ...
  // FIXME: a more parameterized grad. of lambda error
  adapt_wavelet_limited ((scalar *){ omega, velFluidNorm, velAirNorm, gltn}, {f}, (double[]){ OMEGA_ERR, UFLUID_ERR, UAIR_ERR, 1.e-3}, refRegion, minlevel = MINLEVEL);
  refine(y<=(7.550*WAVE_LENGTH/pow(2, MAXLEVEL)) && level<MAXLEVEL);
