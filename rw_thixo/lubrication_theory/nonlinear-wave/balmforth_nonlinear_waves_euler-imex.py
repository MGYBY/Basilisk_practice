...
class BalmforthNonlinearStable:
    ...
    def diffuse_step(self, rhs_int, h_new, dt):
        alpha = self.kappa / (self.T * np.maximum(h_new,1e-8)**2)
        tmp = (self.L_evecs_inv @ rhs_int.T).T
        denom = 1.0 - dt * alpha[:,None] * self.L_evals[None,:]
        tmp = tmp / denom
    ...
    def solve(self, *, t_end: float, dt_max: float, n_eval: int=31, checkpoint_dir: Optional[Path]=None, progress_every: int=100):
        ...
        h_hist[0]=h; lam_hist[0]=self.assemble_lambda_full(lam_int); ms[0]=np.mean((h-1.0)**2)
        snapshot_indices=np.array(sorted(set(np.linspace(0,n_eval-1,min(7,n_eval),dtype=int))))
        if self.logger: self.logger.log(f"Stored initial state at physical time t=0; <(h-1)^2>={ms[0]:.6e}")
        t=0.0; out_index=1; step=0
        while t < t_end - 1e-14:
            dt, dt_x, dt_z, umax, numax = self.suggested_dt(h, lam_int, dt_max)
            if t + dt > t_end: dt = t_end - t
            h_rhs, lam_rhs, _, _ = self.explicit_rhs(h, lam_int)
            # IMEX Euler, conservative choice for robustness
            h_new = self.filter_x(h + dt*h_rhs)
            h_new = np.maximum(h_new, 1e-6)
            rhs_lam = self.filter_x(lam_int + dt*lam_rhs)
            lam_new = self.diffuse_step(rhs_lam, h_new, dt)
            ...
            if (step % progress_every == 0) or step == 1 or abs(t-t_end) < 1e-12:
                ...
                self.logger.log(f"Step {step}: physical time t={t:.6g}/{t_end:.6g}, <(h-1)^2>={ms_now:.6e}, dt={dt:.3g}, dt_x={dt_x:.3g}, dt_z={dt_z:.3g}, umax={umax:.3g}")
            while out_index < n_eval and t >= times_out[out_index] - 1e-14:
                lam_full = self.assemble_lambda_full(lam_int)
                h_hist[out_index]=h; lam_hist[out_index]=lam_full; ms[out_index]=np.mean((h-1.0)**2)
                self.logger.log(f"Stored output at physical time t={times_out[out_index]:.6g}; <(h-1)^2>={ms[out_index]:.6e}")
                if checkpoint_dir is not None:
                    checkpoint_dir.mkdir(parents=True, exist_ok=True)
                    np.savez_compressed(checkpoint_dir/'solution_checkpoint.npz', x=self.x, zeta=self.zeta, times=times_out[:out_index+1],
                                        h=h_hist[:out_index+1], lambda_full=lam_hist[:out_index+1], mean_square=ms[:out_index+1],
...
def parse_args(argv=None):
    p=argparse.ArgumentParser(description='Stable nonlinear solver for Balmforth (2025), Section 5 / Appendix F.')
...
def main(argv=None):
    ...
    solver=BalmforthNonlinearStable(a=args.a,Gamma=args.Gamma,T=args.T,kappa=args.kappa,k_wave=args.k_wave,N=args.N,M=args.M,amplitude=args.amplitude,logger=logger)
