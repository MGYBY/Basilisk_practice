...
#endif
#ifndef DOMAINLENGTH
#define DOMAINLENGTH 100.00
#endif
#ifndef SIMTIME
#define SIMTIME 204.0
#endif
...
#define PLOTRANGEMAX (3.50*normalDepth)
#define distWavelengh 2.0
...
event init (i = 0)
{
  ...
  foreach() {
    zb[] = 0.;
    // TODO: also use perturbation function here (fixed)
    // h[] = (x<=distWavelengh && x>=distWavelengh/2.0)?H0*(1. + AMP*sin (2.*pi*x/distWavelengh)):H0;
    h[] = H0*(1.0+AMP*perturbation (x, 0.0, distWavelengh));
    eta[] = zb[] + h[];
    ...
    for (int l = 0; l < nl; l++) {
      ...
      double pu = perturbation (x, 0.0, distWavelengh);
      vector uk = ul[l];
      scalar lam = lambdal[l];
      // TODO: add perturbation to velocity too using zeta coordinate (fixed)
      // uk.x[] = interp_avg (zprof, uprof, nprof, z0, z1);
      double ubase = interp_avg (zprof, uprof, nprof, z0, z1);
      uk.x[] = ubase*sqrt((1. + UAMP*pu)); // constant Fr disturbance
...
event adapt1 (t=6; i++) {
  ...
  adapt_wavelet({h, depthGrad, uAve}, (double[]){normalDepth/60.0, 0.02, normalVel/60.0}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
