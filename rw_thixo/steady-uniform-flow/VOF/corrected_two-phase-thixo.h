...
static inline double thixo_D2_point (Point point)
{
  ...
  return fabs(dxx);
#elif dimension == 2
  double dxx = (u.x[] - u.x[-1,0])/(2.0*Delta);
  double dyy = ((u.y[0,1]-u.y[0,-1])+(u.y[-1,1]-u.y[-1,-1]))/(4.0*Delta);
  double dxy = 0.5*(((u.x[0,1]-u.x[0,-1])+(u.x[-1,1]-u.x[-1,-1]))/4.0 + (u.y[] - u.y[-1,0]))/Delta;
