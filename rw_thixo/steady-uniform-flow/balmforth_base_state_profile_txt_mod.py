#!/usr/bin/env python3
"""
Solve the steady-uniform base state from Section 3 of

    Balmforth (2025), Instabilities in falling films of thixotropic fluids

for one fixed set of parameters (a, Gamma, kappa>0), and write a plain text file
with three columns:

    z   U(z)   Lambda(z)

No headers are written. Columns are separated by spaces.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Solve one positive-kappa base state and write z U Lambda to a text file.')
    ...
    p.add_argument('--n-rows', type=int, default=201, help='Number of rows written to the output text file.')
...
def main() -> None:
    ...
    np.savetxt(outpath, out, fmt='%.16e')
