...
void kurganov (double hm, double hp, double um, double up, double tem, double tep, double sphim, double sphip, double Delta, double * fh, double * fq, double * fhte, double * fhsphi, double * dtmax)
{
  // φ+Φ
...
//   double phim = (2.*hiem-hm*(G*hm+pow(um,2.0)))/pow(hm,3.0), phip = (2.*hiep-hp*(G*hp+pow(up,2.0)))/pow(hp,3.0);
  ...
  double hsphim = hm*sphim, hsphip = hp*sphip; // the conservative variables for h*φ
  double a = max(ap, -am);
  ...
  if (a > epsilon) {
    ...
    *fhsphi = (ap*(qm*sphim) - am*(qp*sphip) + ap*am*(hsphip - hsphim))/(ap - am);
...
void kurganovSharp (double hm, double hp, double um, double up, double tem, double tep, double sphim, double sphip, double Delta, double * fh, double * fq, double * fhte, double * fhsphi, double * dtmax)
{
  // φ+Φ
  double ptp = -1.0*((G*hp-2.0*tep+up*up)/pow(hp,2.0)), ptm = -1.0*((G*hm-2.0*tem+um*um)/pow(hm,2.0)); // phi-total variable
  // Π variable
...
//   double phim = (2.*hiem-hm*(G*hm+pow(um,2.0)))/pow(hm,3.0), phip = (2.*hiep-hp*(G*hp+pow(up,2.0)))/pow(hp,3.0);
  ...
  if (a > epsilon) {
    ...
    *fhte = (ap*(qm*tem+pm*um) - am*(qp*tep+pp*up) + ap*am*(hEp - hEm))/(ap - am) + (ap*am*((hEp - hEm)/(ap - am)-qCorr));
    ...
    wint = (ap*hsphip-am*hsphim-((qp*sphip)-(qm*sphim)))/(ap-am);
    qCorr = mmo((hsphip-wint)/(ap-am), (wint-hsphim)/(ap-am));
    *fhsphi = (ap*(qm*sphim) - am*(qp*sphip) + ap*am*(hsphip - hsphim))/(ap - am) + (ap*am*((hsphip - hsphim)/(ap - am)-qCorr));
...
void hllc (double hm, double hp, double um, double up, double tem, double tep, double sphim, double sphip, double Delta, double * fh, double * fq, double * fhte, double * fhsphi, double * dtmax)
{
  double ptp = -1.0*((G*hp-2.0*tep+up*up)/pow(hp,2.0)), ptm = -1.0*((G*hm-2.0*tem+um*um)/pow(hm,2.0)); // phi-total variable
  double pp = 0.50*G*hp*hp+ptp*pow(hp, 3.), pm = 0.50*G*hm*hm+ptm*pow(hm, 3.); // P variable
  double qm = hm*um, qp = hp*up;
  ...
  if (0. <= SL) {
    ...
    *fhsphi = qp*sphip;
    ...
    double fhtem = qm*tem+pm*um;
    double fhsphim = qm*sphim;
    double fhp = up*hp;
    double fup = hp*(up*up + G*hp/2.);
    double fhtep = qp*tep+pp*up;
    double fhsphip = qp*sphip;

    *fh = (SR*fhm - SL*fhp + SL*SR*(hp - hm))/(SR - SL);
    *fq = (SR*fum - SL*fup + SL*SR*(qp - qm))/(SR - SL);
    *fhte = (SR*fhtem - SL*fhtep + SL*SR*(hEp - hEm))/(SR - SL);
    *fhsphi = (SR*fhsphim - SL*fhsphip + SL*SR*(hsphip - hsphim))/(SR - SL);
  }
