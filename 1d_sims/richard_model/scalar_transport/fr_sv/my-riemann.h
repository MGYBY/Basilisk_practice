...
// void kinetic (double hm, double hp, double concm, double concp, double um, double up, double Delta,
// 	      double * fh, double * fhconc, double * fq, double * dtmax)
// {
...
void kurganov (double hm, double hp, double concm, double concp, double um, double up, double Delta,
	      double * fh, double * fhconc, double * fq, double * dtmax)
{
  ...
  double hconcm = hm*concm, hconcp = hp*concp;
  double a = max(ap, -am);
  if (a > epsilon) {
    *fh = (ap*qm - am*qp + ap*am*(hp - hm))/(ap - am); // (4.5) of [1]
    *fq = (ap*(qm*um + G*sq(hm)/2.) - am*(qp*up + G*sq(hp)/2.) + 
	    ap*am*(qp - qm))/(ap - am);
    *fhconc = (ap*concm*um - am*concp*up + ap*am*(concp - concm))/(ap - am);
