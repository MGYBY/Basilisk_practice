				  ...
				  scalar * updates, double dt)
{
  ...
  scalar conci = input[1], conco = output[1], dhconc = updates[1];
  vector * uol = (vector *) &output[2];
  ...
  foreach() {
    ...
    double concold = conci[];
    // conco[] = (hold*concold + dt*dhconc[])/conc[];
    conco[] = concold + dt*dhconc[];

    if (ho[] > dry) {
      for (int l = 0; l < nl; l++) {
        vector uo = vector(output[2 + dimension*l]);
      	vector ui = vector(input[2 + dimension*l]),
	  dhu = vector(updates[2 + dimension*l]);
	foreach_dimension()
...
double update_saint_venant (scalar * evolving, scalar * updates, double dtmax)
{
  ...
  scalar conc = evolving[1], dhconc = updates[1];
  ...
  *Fh* and *Fq* will contain the fluxes for $h$ and $h\mathbf{u}$
  ...
  vector gh[], geta[], gconc[];
  tensor gu[];
  for (scalar s in {gh, gconc, geta, gu}) {
    s.gradient = zero;
    #if TREE
      s.prolongation = refine_linear;
    #endif
  }
  gradients ({h, conc, eta, u}, {gh, gconc, geta, gu});
  ...
  for (int l = 0; l < nl; l++) {
    ...
    vector u = vector (evolving[2 + dimension*l]);
    if (l > 0)
    ...
    double concm = conc[-1] + dx*gconc.x[-1];
    double concp = conc[] - dx*gconc.x[];
	...
	double fh, fhconc, fu, fv;
	kurganov (hm, hp, concm, concp, um, up, Delta*cm[]/fm.x[], &fh, &fhconc, &fu, &dtmax);
    // FIXME: modify HLLC solver in the future
	...
	Fh.x[]   = fm.x[]*fh;
    Fhconc.x[]   = fm.x[]*fhconc;
	Fq.x.x[] = fm.x[]*(fu - sl);
	S.x[]    = fm.x[]*(fu - sr);
	Fq.y.x[] = fm.x[]*fv;
      }
      else // dry
	Fh.x[] = 0., Fhconc.x[] = 0., Fq.x.x[] = S.x[] = Fq.y.x[] = 0.;
    }
    ...
    vector dhu = vector(updates[2 + dimension*l]);
    foreach() {
      double dhl =
    ...
    double dhconcl =
	layer[l]*(Fhconc.x[1,0] - Fhconc.x[] + Fhconc.y[0,1] - Fhconc.y[])/(cm[]*Delta);
      dhconc[] = - dhconcl + (l > 0 ? dhconc[] : 0.);
...
event defaults (i = 0)
{
  ...
  evolving = list_concat ({h, conc}, (scalar *) ul);
  foreach()
