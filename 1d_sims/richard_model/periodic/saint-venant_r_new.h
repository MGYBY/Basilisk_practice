				  ...
				  scalar * updates, double dt)
{
  ...
  scalar tei = input[1], teo = output[1], dhte = updates[1];
  // FIXME: for sphi in 2D, this may not be true for three components
  scalar sphii = input[2], sphio = output[2], dhsphi = updates[2];
  vector * uol = (vector *) &output[3];
  ...
  foreach() {
    ...
    double teold = tei[];
    teo[] = (hold*teold + dt*dhte[])/ho[];

    double sphiold = sphii[];
    sphio[] = (hold*sphiold + dt*dhsphi[])/ho[];

    if (ho[] > dry) {
      for (int l = 0; l < nl; l++) {
        vector uo = vector(output[3 + dimension*l]);
      	vector ui = vector(input[3 + dimension*l]),
	  dhu = vector(updates[3 + dimension*l]);
	foreach_dimension()
...
double update_saint_venant (scalar * evolving, scalar * updates, double dtmax)
{
  ...
  scalar te = evolving[1], dhte = updates[1];
  scalar sphi = evolving[2], dhsphi = updates[2];
  vector u = vector(evolving[3]);
  ...
  *Fh* and *Fq* will contain the fluxes for $h$ and $h\mathbf{u}$
  ...
  vector gh[], gte[], geta[], gsphi[];
  tensor gu[];
  for (scalar s in {gh, gte, gsphi, geta, gu}) {
    s.gradient = zero;
    #if TREE
      s.prolongation = refine_linear;
    #endif
  }
  gradients ({h, te, sphi, eta, u}, {gh, gte, gsphi, geta, gu});
  ...
  for (int l = 0; l < nl; l++) {
    ...
    vector u = vector (evolving[3 + dimension*l]);
    if (l > 0)
    ...
    double tem = te[-1] + dx*gte.x[-1,0];
    double tep = te[] - dx*gte.x[];

    double sphim = sphi[-1] + dx*gsphi.x[-1,0];
    double sphip = sphi[] - dx*gsphi.x[];
	...
	double fh, fu, fv, fhte, fhsphi;
// 	kurganov (hm, hp, um, up, Delta*cm[]/fm.x[], &fh, &fu, &dtmax);
//     kurganov (hm, hp, um, up, tem, tep, sphim, sphip, Delta*cm[]/fm.x[], &fh, &fu, &fhte, &fhsphi, &dtmax);
//     kurganovSharp (hm, hp, um, up, tem, tep, sphim, sphip, Delta*cm[]/fm.x[], &fh, &fu, &fhte, &fhsphi, &dtmax);
    hllc (hm, hp, um, up, tem, tep, sphim, sphip, Delta*cm[]/fm.x[], &fh, &fu, &fhte, &fhsphi, &dtmax);
...
// 	double sr = G/2.*(sq(hm) - sq(hr) + (hr + hn)*(zn - zr));
	...
	Fh.x[]   = fm.x[]*fh;
    Fhte.x[]   = fm.x[]*fhte;
    Fhsphi.x[] = fm.x[]*fhsphi;
	Fq.x.x[] = fm.x[]*(fu - sl);
	S.x[]    = fm.x[]*(fu - sr);
	Fq.y.x[] = fm.x[]*fv;
      }
      else // dry
	Fh.x[] = 0., Fhte.x[] = 0., Fhsphi.x[] = 0., Fq.x.x[] = S.x[] = Fq.y.x[] = 0.;
    }
    ...
    vector dhu = vector(updates[3 + dimension*l]);
    foreach() {
      double dhl =
	layer[l]*(Fh.x[1,0] - Fh.x[] + Fh.y[0,1] - Fh.y[])/(cm[]*Delta);
      ...
      double dhtel =
	layer[l]*(Fhte.x[1,0] - Fhte.x[] + Fhte.y[0,1] - Fhte.y[])/(cm[]*Delta);
      dhte[] = - dhtel + (l > 0 ? dhte[] : 0.);

      double dhsphil =
	layer[l]*(Fhsphi.x[1,0] - Fhsphi.x[] + Fhsphi.y[0,1] - Fhsphi.y[])/(cm[]*Delta);
      dhsphi[] = - dhsphil + (l > 0 ? dhsphi[] : 0.);
...
event defaults (i = 0)
{
  ...
  evolving = list_concat ({h, te, sphi}, (scalar *) ul);
  foreach()
