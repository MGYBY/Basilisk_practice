...
scalar hGrad[], qGrad[], hteGrad[], hsphiGrad[];
char s[80];
...
//     sphi[] = gHat/totalDepth/pow(kappa,2.0);
    hsphi[] = h[]*gHat/normalDepth/pow(kappa,2.0);
    hte[] = h[]*(0.50*pow((q[]/h[]),2.0)+0.5*gPrime*h[]+0.5*pow(h[],2.0)*(bigPhiNVal+hsphi[]/h[]));
    }
}
...
//   }
//
//   adapt_wavelet({h, hGrad, qGrad, hteGrad, hsphiGrad}, (double[]){normalDepth/350.0, 7.0, 7.0, 7.0, 7.0}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
