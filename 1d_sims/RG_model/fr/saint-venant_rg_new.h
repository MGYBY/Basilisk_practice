...
double update_saint_venant (scalar * evolving, scalar * updates, double dtmax)
{
  ...
  *Fh* and *Fq* will contain the fluxes for $h$ and $h\mathbf{u}$
  ...
  vector gh[], gte[], geta[];
  tensor gu[];
  for (scalar s in {gh, gte, geta, gu}) {
    s.gradient = zero;
    #if TREE
      s.prolongation = refine_linear;
    #endif
  }
  gradients ({h, te, eta, u}, {gh, gte, geta, gu});
  ...
  for (int l = 0; l < nl; l++) {
	...
	double fh, fu, fv, fhte;
// 	kurganov (hm, hp, um, up, Delta*cm[]/fm.x[], &fh, &fu, &dtmax);
//     kurganov (hm, hp, um, up, tem, tep, Delta*cm[]/fm.x[], &fh, &fu, &fhte, &dtmax);
    kurganovSharp (hm, hp, um, up, tem, tep, Delta*cm[]/fm.x[], &fh, &fu, &fhte, &dtmax);
...
// 	double sr = G/2.*(sq(hm) - sq(hr) + (hr + hn)*(zn - zr));
	...
	Fh.x[] = 0., Fhte.x[] = 0., Fq.x.x[] = S.x[] = Fq.y.x[] = 0.;
    }
...
event defaults (i = 0)
{
  ...
  evolving = list_concat ({h, te}, (scalar *) ul);
  foreach()
