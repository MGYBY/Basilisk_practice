...
#define normalVel (froude*pow(gravity*cosTheta*normalDepth,0.50)) // usual definition 1D
...
// we use the dimensionless length $\sin(\theta)L_x/H=1200$
/* vertical fall for 1D ($\sin(\theta)$) */
...
char s[80];
scalar hmax[], hGrad[], uxGrad[], teGrad[], uAve[], phiSum[], phiSumGrad[];
...
event init (i = 0) {
  ...
  u.n[left] = dirichlet(distVelInlet(t));
  te[left] = dirichlet(0.50*pow(distVelInlet(t),2.0)+0.50*(G*distDepthInlet(t)+(bigPhiVal+smallPhiVal)*pow(distDepthInlet(t),2.0)));
...
//     vMed = (3. / 4.) * u.y[] + (1. / 4.) * vMed + (1. / 4.) * dt * momYFric(uPrev, vMed, h[], te[]);
    teMed = (3. / 4.) * te[] + (1. / 4.) * teMed + (1. / 4.) * dt * teFric(uPrev, 0.0, h[], teMed); // FIXME: maybe not general enough for 2D in v
...
event adapt1 (t=0; t<=initRefineStage; i++) {
  vector gh[], gux[], gte[], gPhiSum[];

  foreach()
  {
    phiSum[] = -1.0*((G*h[]-2.0*te[]+pow(u.x[],2.0))/pow(h[],2.0));
  }

  gradients ({h}, {gh});
  gradients ({u.x}, {gux});
//   gradients ({te}, {gte});
  gradients ({phiSum}, {gPhiSum});
  ...
  foreach()
  {
    ...
    uxGrad[] = norm(gux);
//     teGrad[] = norm(gte);
    phiSumGrad[] = norm(gPhiSum);
  }

  adapt_wavelet({h, hGrad, uxGrad, phiSumGrad}, (double[]){normalDepth/500.0, 0.0025, 0.0025, 0.0025}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
//   adapt_wavelet({h}, (double[]){normalDepth/250.0}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
...
//   fprintf(stderr, "# refined %d cells, coarsened %d cells\n", s.nf, s.nc);
  ...
  refine(x<=(1.25*distPeriod*normalVel) && t<=(distPeriod) && level<MAXLEVEL);
...
//   adapt_wavelet({h, hGrad, uxGrad, teGrad}, (double[]){normalDepth/400.0, 0.0025, normalVel/400.0, min((normalVel*normalVel), (G*normalDepth))/350.0}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  adapt_wavelet({h}, (double[]){normalDepth/200.0}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
//      astats s = adapt_wavelet({h, depthGrad}, (double[]){1.0/300.0, 0.00016}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  // astats s = adapt_wavelet({ depthGrad}, (double[]){ 0.00010}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
//   fprintf(stderr, "# refined %d cells, coarsened %d cells\n", s.nf, s.nc);
