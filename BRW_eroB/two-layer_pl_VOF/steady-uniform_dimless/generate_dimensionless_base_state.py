...
The input is an INI file containing the independent dimensionless groups
documented in `README.md` and the formulation-report addendum. The lower
consistency coefficient
Lambda_l is *derived* from the steady-uniform compatibility condition

    integral_0^1 U_l(z) dz = 1,

and Lambda_u = kappa_K Lambda_l.  The ambient Newtonian viscosity is then
...
characteristic dimensionless shear rate gamma=1:

    eta_air = eta_upper(gamma=1) / R_mu,u/a.

The script writes ordinary
...
def derive_air_viscosity(c: Case, lam_upper: float) -> tuple[float, float]:
    """Return `eta_u(gamma*=1)` and `eta_a = eta_u/R_mu,u/a`.

    The upper value uses the complete selected constitutive model, including
    yield-stress terms, shear-rate safeguards, and final viscosity clipping.
    Both returned dynamic viscosities are dimensionless.
    """
...
def derive_lambda_lower(c: Case) -> float:
    """Solve the lower mean-velocity compatibility condition for Lambda_l.

    If Gamma_l(tau; Lambda_l) is the positive inverse of the complete lower
    constitutive relation, the numerical root satisfies

        integral_0^1 (1-z) Gamma_l(tau_l(z); Lambda_l) dz = 1.
    """
...
def write_generated_header(path: Path, c: Case, base: dict) -> None:
    ...
    with path.open("w", encoding="utf-8") as stream:
        stream.write(
            ...
            "   CASE_LAMBDA_LOWER is the lower mean-flow compatibility root.\n"
            "   CASE_LAMBDA_UPPER = CASE_CONSISTENCY_RATIO*CASE_LAMBDA_LOWER.\n"
            "   CASE_AIR_ETA = CASE_UPPER_REFERENCE_ETA /\n"
            "                  CASE_UPPER_TO_AIR_VISCOSITY_RATIO. */\n"
        )
