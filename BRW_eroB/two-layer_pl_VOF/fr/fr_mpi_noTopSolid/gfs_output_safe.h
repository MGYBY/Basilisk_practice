...
all zero because it must carry the Gerris leaf flag.  On POSIX systems the repair moves the final two-byte delimiter and truncates
in place; it does not reread and rewrite the complete GFS file. The former
stream-copy method is retained as a non-POSIX/explicit compatibility fallback.
The writer, selected variables, tree, and padding-detection condition are unchanged.
*/
#ifndef BASILISK_GFS_OUTPUT_SAFE_H
#define BASILISK_GFS_OUTPUT_SAFE_H

#ifndef CASE_GFS_FAST_TRUNCATE
# if defined(__unix__) || defined(__APPLE__)
#  define CASE_GFS_FAST_TRUNCATE 1
# else
#  define CASE_GFS_FAST_TRUNCATE 0
# endif
#endif
#if CASE_GFS_FAST_TRUNCATE
# include "gfs_tail_truncate.h"
#endif

static void gfs_output_safe_fatal (const char * message)
{
  fprintf (ferr, "gfs-output-safe: %s\n", message);
  fflush (ferr);
#if _MPI
                                       ...
                                       long prefix_length)
{
  ...
  if (!buffer || !temporary) {
    ...
    free (buffer);
    free (temporary);
    gfs_output_safe_fatal ("file repair failed; see preceding error");
  }
                                             ...
                                             scalar * fields)
{
  ...
  if (pid() == 0) {
    ...
    if (all_zero) {
#if CASE_GFS_FAST_TRUNCATE
      if (gfs_tail_truncate (filename, record_start) != 0)
        gfs_output_safe_fatal ("cannot truncate the verified padding record");
#else
