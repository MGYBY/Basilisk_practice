...
astats adapt_wavelet_limited (struct Adapt_limited p)
{
  ...
  foreach_cell() {
    ...
    /* MPI traversal fix (v2p6): `local` only tests this cell and its
       IMMEDIATE children. An active nonlocal ancestor can have locally
       owned grandchildren or deeper descendants. In foreach_cell(),
       `continue` prunes the whole subtree, so it is NOT valid here.
       Gate the marking work, but always allow descent, as in upstream
       grid/tree-common.h::adapt_wavelet(). */
    if (local) {
      int i = 0;
