/**
# Dimensionless total-momentum-consistent VOF advection for nested three phases
...
Here rho1, rho2 and rho3 are density ratios and u is scaled by the lower-layer mean velocity. Each term is transported as a VOF-confined tracer of the corresponding cumulative interface. Their signed sum is the dimensionless total momentum of the lower liquid, upper liquid and ambient phase.
*/
...
event vof (i++)
{
  /* The custom q boundary/prolongation callbacks read fractions AND velocity.
     Synchronize them before constructing temporary momentum tracers. */
  boundary ({f_lower, f});
  boundary ((scalar *){u});
  ...
  for (scalar s in {q1, q2lower, q2total, q3}) {
    ...
    for (scalar uc in (scalar *){u})
      s.depends = list_add (s.depends, uc);
    foreach_dimension()
      s.v.x.i = -1;
  }
