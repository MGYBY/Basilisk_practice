...
advection, quadtree AMR, optional capillarity and phase-selective gravity,
localized constant-Froude initialization, Fourier amplitude tracking,
vertical slices, interface facets, GFS output and dumps.

The former embedded ceiling is removed. Air extends to y=LX, where a distant
slip/symmetry boundary closes the box. The former aligned ceiling height is
retained only as a near-air initialization/matching and output height.

v2p7 is based on the v2p6 top-solid branch, retaining its corrected MPI AMR
traversal and in-place GFS padding repair. See README.md for provenance and
for the limitations of a geometry-plus-viscous-operator comparison.
*/

#include "case_runtime_options.h"

#include "grid/quadtree.h"
#include "adapt_wavelet_leave_interface_limited.h"
#include "navier-stokes/centered.h"
#if EMBED
# error "v2p7 requires the non-embedded centered/viscosity.h solver path"
#endif
...
#include "front_runner_ic.h"
#include "no_top_solid_helpers.h"

/* Monitoring controls are in case_runtime_options.h (production by default). */
...
/* Derived by the generator from the upper-reference/air viscosity ratio. */
...
/* Not a boundary: this is the retained near-air matching/output height.
   The actual outer top is Y0 + L0 = LX. */
static double near_air_y = NOMINAL_HEIGHT;
static double u_reference = 1.;
static int last_nf = 0, last_nc = 0;
static FILE * wave_amplitude_fp = NULL;
static FrontRunnerIC front_runner_ic;

#include "mpi_case_diagnostics.h"
...
/* No embedded surface or mask. The bed remains no-slip. At y=LX the
   outer top is impermeable and shear-free, NOT an open-atmosphere outlet.
   Keep centered.h's acceleration-consistent pressure Neumann condition;
   imposing p=0 here as well would over-specify the closed boundary. */
...
static void validate_case_parameters (void)
{
  ...
  if (G_ACCEL <= 0. || SLOPE_TAN <= 0. || H1 <= 0. || H2 <= 0. ||
      LX <= 0. || NOMINAL_HEIGHT <= H1 + H2 || NOMINAL_HEIGHT >= LX ||
      RHOLOWERLAYER <= 0. || RHOUPPERLAYER <= 0. || AIRRHO <= 0. ||
      K_LOWER <= 0. || K_UPPER <= 0. || N_LOWER <= 0. || N_UPPER <= 0. ||
      MUMAXLOWER <= 0. || MUMAXUPPER <= 0. || AIRMU <= 0. ||
      CASE_INTERFACIAL_APPARENT_VISCOSITY_RATIO <= 0. ||
      CASE_DERIVED_CONSISTENCY_RATIO <= 0. ||
      CASE_UPPER_TO_AIR_VISCOSITY_RATIO <= 0. ||
      CASE_UPPER_VISCOSITY_REFERENCE_SHEAR <= 0. ||
      CASE_UPPER_REFERENCE_ETA <= 0. ||
      GAMMA_MIN <= 0. || SOLVER_TOLERANCE <= 0. || SOLVER_NITERMAX <= 0 ||
      MAX_DT <= 0. || CFL_NUMBER <= 0. ||
      !isfinite(CASE_FAR_AIR_CELL_GROWTH) || CASE_FAR_AIR_CELL_GROWTH < 0.) {
    fprintf (ferr,
             "One or more geometry/material/solver parameters are invalid. "
             "Check case_parameters.ini and regenerate the base state.\n");
    exit (1);
  }
  if (fabs(CASE_REALIZED_INTERFACE_VISCOSITY_RATIO/
           CASE_INTERFACIAL_APPARENT_VISCOSITY_RATIO - 1.) > 1.e-8 ||
      fabs(CASE_INTERFACE_TRACTION_MISMATCH) > 1.e-8) {
    fprintf (ferr,
             "Generated R_eta,I mapping failed validation: requested=%g "
             "realized=%g tractionMismatch=%g. Regenerate the base state.\n",
             (double)CASE_INTERFACIAL_APPARENT_VISCOSITY_RATIO,
             (double)CASE_REALIZED_INTERFACE_VISCOSITY_RATIO,
             (double)CASE_INTERFACE_TRACTION_MISMATCH);
    exit (1);
  }
...
static void validate_simple_text_profiles (void)
{
  ...
  const double xtol = 1.e-10*max(1., near_air_y);
  ...
  if (fabs(profile_u0.x[0]) > xtol ||
      fabs(profile_u0.x[profile_u0.n - 1] - near_air_y) > xtol) {
    fprintf (ferr,
             "Near-field profiles must cover y=0 to the matching height=%g; "
             "got [%g,%g].\n"
             "Regenerate the dimensionless base state.\n",
             near_air_y, profile_u0.x[0], profile_u0.x[profile_u0.n - 1]);
    exit (1);
  }
  ...
  const double uceil = profile1d_eval (&profile_u0, near_air_y);
  const double pbed_file = profile1d_eval (&profile_p, 0.);
  const double pceil = profile1d_eval (&profile_p, near_air_y);
  ...
  if (fabs(ubed) > utol || fabs(uceil - us_expected) > utol ||
    ...
    fprintf (ferr,
             ...
             "  matching-height velocity: file=%g expected=%g\n"
             "  ambient spread:     %g\n"
             "  bed pressure:       file=%g expected=%g\n"
             "  bed velocity / matching-height pressure datum: %g / %g\n",
  ...
  fprintf (ferr,
           ...
           "U_ambient=%g p_bed=%g matchingHeight=%g\n",
           ui_file, us_file, uceil, pbed_file, near_air_y);
  fprintf (ferr,
           "# groups: Fr_l=%g S0=%g h_r=%g r_rho=%g "
           "R_eta_I=%g R_eta_I_realized=%g kappa_K_derived=%g "
           "Lambda_l=%g Lambda_u=%g epsilon_l=%g epsilon_u=%g "
           "gamma_l_I=%g gamma_u_I=%g eta_l_I=%g eta_u_I=%g "
           "r_air=%g R_mu_u_air=%g gamma_ref_u=%g "
           "eta_u_ref=%g eta_air=%g\n",
           ...
           (double)CASE_LOWER_EPSILON, (double)CASE_UPPER_EPSILON,
           (double)CASE_LOWER_INTERFACE_GAMMA,
           (double)CASE_UPPER_INTERFACE_GAMMA,
           (double)CASE_LOWER_INTERFACE_ETA,
           (double)CASE_UPPER_INTERFACE_ETA,
           (double)CASE_AIR_DENSITY_RATIO,
           (double)CASE_UPPER_TO_AIR_VISCOSITY_RATIO,
           (double)CASE_UPPER_VISCOSITY_REFERENCE_SHEAR,
           (double)CASE_UPPER_REFERENCE_ETA,
           (double)CASE_AIR_ETA);
}

/** Shape shared by both material interfaces: localized hump or legacy sine. */
static inline double disturbance_sine (double xx)
{
#if CASE_FRONT_RUNNER_ENABLED
  return fr_bump(&front_runner_ic, xx);
#else
...
/** Compatible base velocity plus an optional divergence-free perturbation.
    ...
    and matching plane, and is zero above that plane initially. This is
    an initial field, not an internal boundary enforced during the run. */
static inline void initial_velocity_components (double xx, double yy,
                                                double * ux, double * uy)
{
#if CASE_FRONT_RUNNER_ENABLED
  fr_velocity(&front_runner_ic, xx, yy, ux, uy);
  return;
#endif
  if (yy >= near_air_y) {
    *ux = profile1d_eval (&profile_u0, near_air_y);
    *uy = 0.;
    return;
  }
  const double mode = PERTURBATION_MODE_NUMBER;
  const double phase = 2.*pi*mode*xx/LX + PERTURBATION_PHASE;
  const double sy = clamp(yy/(near_air_y + SEPS), 0., 1.);
  ...
  *uy = -amp*mode*(near_air_y/LX)*
    (1. - cos(2.*pi*sy))*cos(phase);
}

/** Piecewise hydrostatic pressure for the locally perturbed interfaces,
    retaining the old hydrostatic reference at the matching plane.
    Negative gauge pressure above that plane is intentional, not a vacuum.
    The pressure gradient remains -rho_air/Fr_l^2 in the extended air. */
static inline double local_hydrostatic_pressure (double xx, double yy)
{
  ...
  if (yy > eta2)
    return AIRRHO*gn*(near_air_y - yy);
  if (yy > h1x)
    return AIRRHO*gn*(near_air_y - eta2) +
      RHOUPPERLAYER*gn*(eta2 - yy);
  return AIRRHO*gn*(near_air_y - eta2) +
    RHOUPPERLAYER*gn*h2x + RHOLOWERLAYER*gn*(h1x - yy);
}

/* Mode 0 reads the old table below the matching plane and extends its
   hydrostatic slope above it; profile1d_eval alone would clamp to p=0. */
static inline double extended_base_pressure (double yy)
{
  if (yy > near_air_y)
    return nts_air_pressure (yy, near_air_y,
                             profile1d_eval (&profile_p, near_air_y),
                             AIRRHO, G_ACCEL*COS_THETA);
  return profile1d_eval (&profile_p, yy);
}

/** Keep the old near-field maximum levels. Grade the newly active far air
    to larger cells instead of permitting level MAXLEVEL-4 everywhere. */
static int local_maxlevel_cell (double xx, double yy, double zz,
                                double delta)
{
  (void) xx; (void) zz;
  return nts_maxlevel (yy, delta, near_air_y, CEILING_FINE_FRACTION,
                       finest_delta(), MAXLEVEL, MINLEVEL,
                       CEILING_LEVEL_DROP, CASE_FAR_AIR_CELL_GROWTH);
}
...
static inline void force_required_refinement (void)
{
  ...
  /* A footprint test reaches the same prescribed basal strip even when
     starting from coarse cells whose centres lie above the strip. */
  refine (y - 0.5*Delta < 4.5*dmin && level < MAXLEVEL);
}
...
static void validate_initialization_parameters (void)
{
  ...
  if (max_surface >= near_air_y*CEILING_FINE_FRACTION) {
    fprintf (ferr,
             "The maximum initial free surface (%g) reaches the near-air "
             "coarsening band (%g). Reduce depth amplitude or raise the "
             "matching height.\n",
             max_surface, near_air_y*CEILING_FINE_FRACTION);
    exit (1);
  }
...
{
  /* Both cumulative VOF fields follow the selected (localized or periodic) geometry. */
  fraction (f_lower, initial_internal_interface(x) - y);
  fraction (f, initial_free_surface(x) - y);

  foreach() {
    /* Use explicit field assignments, rather than writes through pointers,
       so qcc can unambiguously track modified velocity fields. */
    double ux0, uy0;
    initial_velocity_components (x, y, &ux0, &uy0);
    u.x[] = ux0;
    u.y[] = uy0;
#if INITIAL_PRESSURE_MODE == 1
    p[] = local_hydrostatic_pressure(x, y);
#else
    p[] = extended_base_pressure(y);
#endif
  }
...
#if CASE_INITIAL_AUDIT
/** Optional initial divergence/discharge audit: absent from production builds. */
static void write_initial_condition_audit (void)
{
  ...
  foreach (reduction(max:divmax) reduction(+:div2) reduction(+:volume)
    ...
    if (cm[] > 1. - 1.e-10 && y > 1.5*Delta &&
        y < near_air_y - 1.5*Delta) {
      const double divu =
...
int main (void)
{
  ...
  /* Preserve the previous aligned height for near-field initialization,
     pressure datum and output only. There are no cut cells at this height. */
  const double dmin = finest_delta();
  near_air_y = ceil(NOMINAL_HEIGHT/dmin - 1.e-12)*dmin;
  ...
  /* Avoid first allocating a uniformly fine square of far-air cells.
     The original INITLEVEL is still applied in the near-field strip. */
  const int seed_level = nts_initial_level (MINLEVEL, INITLEVEL, npe());
  init_grid (1 << seed_level);
  run();
}

event init (i = 0)
{
  case_parallel_banner();
  if (pid() == 0)
...
#if CASE_FRONT_RUNNER_ENABLED
  fr_init(&front_runner_ic, &profile_u0,
          LOWER_DEPTH_PERTURBATION_AMPLITUDE,
          CASE_FR_WAVELENGTH, CASE_FR_CENTER, LIQUID_DEPTH, near_air_y);
  fprintf(ferr, "# front-runner: support=[%g,%g] center=%g wavelength=%g "
                "S0_Lx=%g S0_lambda=%g qLiquid0=%g qAll0=%g\n",
          front_runner_ic.center-front_runner_ic.wavelength/4.,
          front_runner_ic.center+front_runner_ic.wavelength/4.,
          front_runner_ic.center, front_runner_ic.wavelength,
          SLOPE_TAN*LX, SLOPE_TAN*front_runner_ic.wavelength,
          front_runner_ic.q0,
          front_runner_ic.qt + front_runner_ic.us*(LX - near_air_y));
#endif

  validate_initialization_parameters();
  /* Seed only the near-field strip; refine() also supplies the required
     balanced tree transition into the initially coarse far air. */
  refine (y - 0.5*Delta < near_air_y && level < INITLEVEL);
  force_required_refinement();

  for (int pass = 0; pass < INIT_ADAPT_PASSES; pass++) {
    set_initial_fields();
    astats st = perform_adaptation();
#if CASE_VERBOSE_INIT
    const long cells = case_global_leaf_count(); // collective on every rank
    fprintf (ferr, "# init AMR pass %d: nf=%d nc=%d cells=%ld\n",
             pass + 1, st.nf, st.nc, cells);
#endif
    if (st.nf == 0 && st.nc == 0)
      break;
  }
  ...
  const long active_cells = case_global_leaf_count();
  fprintf (ferr,
           "# no-top-solid AMR roll wave: Lx=%g outerTop=%g matchingHeight=%g "
           "dmin=%g MAXLEVEL=%d MINLEVEL=%d activeCells=%ld\n",
           LX, LX, near_air_y, finest_delta(),
           MAXLEVEL, MINLEVEL, active_cells);
  fprintf (ferr,
           "# AMR policy: full MAXLEVEL allowed below y=%g; near-air band max=%d; "
           "basal fine strip=%g; graded far air (cell growth=%g)\n",
           near_air_y*CEILING_FINE_FRACTION,
           max(MINLEVEL, MAXLEVEL - CEILING_LEVEL_DROP),
           4.5*finest_delta(), (double)CASE_FAR_AIR_CELL_GROWTH);
  fprintf (ferr,
  ...
  fprintf (ferr,
           ...
           "farAirMax mgp mgu pRes uRes "
           "V1 V2 V3 Px KE Umax hierarchy bounds "
#if STRATIFIED_VOLUME_AUDIT
           "dVlower dVliquid corrected "
#endif
           "nf nc\n");
}

/* Keep the coefficient synchronization formerly inside the field audit.
   It is a numerical MPI handoff, not diagnostic work. Production does no
   state/face scans here. This hook runs before the native viscosity solve. */
event viscous_term (i++)
{
  face vector muv = mu;
  boundary ((scalar *){alphav, muv});
#if CASE_AUDIT_STEPS > 0
  if (i < CASE_AUDIT_STEPS)
    case_check_state ("pre-viscous", true);
#endif
}

event acceleration (i++)
{
  /* Viscosity has returned by this point. Do not feed a failed velocity solve
     into the pressure projection and obscure the first failure. */
  /* Standard viscosity.h uses TOLERANCE, not embed-only TOLERANCE_MU.
     Its residual is velocity-normalised; do not compare its magnitude
     directly with the old density-weighted embedded residual. */
  case_check_multigrid ("viscosity", mgu, TOLERANCE);
  face vector av = a;
...
event adapt (i++)
{
#if !CASE_FREEZE_AMR_AFTER_INIT
  perform_adaptation();
#endif
}

/* Called after the native projection and centered-velocity correction. */
event end_timestep (i++)
{
  case_check_multigrid ("pressure", mgp, TOLERANCE/sq(dt));
#if CASE_AUDIT_STEPS > 0
  if (i < CASE_AUDIT_STEPS) {
    fprintf (ferr, "# solver-step i=%d t=%g dt=%g mgp=%d mgu=%d "
                   "pRes=%g pTol=%g uRes=%g uTol=%g\n",
             i, t, dt, mgp.i, mgu.i, mgp.resa, TOLERANCE/sq(dt),
             mgu.resa, TOLERANCE);
    case_check_state ("post-projection", false);
  }
#endif
}

#if CASE_STOP_AFTER_STEPS > 0
event case_step_limit (i++, last)
{
  if (i >= CASE_STOP_AFTER_STEPS) {
    case_write_summary ("step-limit");
    fprintf (ferr, "# STOP requested diagnostic step limit: i=%d t=%g\n", i, t);
    return 1;
  }
}
#endif

/** Optional finite-domain guard; it changes no momentum/VOF flux or field.
    A trigger means departure from uniform geometry near a periodic seam,
    not proof of a front-runner crest. It also catches base-state drift. */
#if CASE_FRONT_RUNNER_ENABLED && (CASE_FR_STOP_BEFORE_WRAP || CASE_WRITE_BOUNDARY_GUARD)
event front_runner_boundary_guard (i += 1000)
{
  double edge_departure = 0.;
  foreach(reduction(max:edge_departure)) {
    if (cm[] > 0. && (x < CASE_FR_GUARD_WIDTH || x > LX-CASE_FR_GUARD_WIDTH)) {
      for (scalar ff in {f_lower, f}) {
        if (ff[] > INTERFACE_EPS && ff[] < 1.-INTERFACE_EPS) {
          coord normal = interface_normal(point, ff), centroid;
          const double alpha = plane_alpha(ff[], normal);
          plane_area_center(normal, alpha, &centroid);
          const double height = y + Delta*centroid.y;
          const double reference = ff.i == f_lower.i ? H1 : LIQUID_DEPTH;
          edge_departure = max(edge_departure, fabs(height-reference));
        }
      }
    }
  }
#if CASE_WRITE_BOUNDARY_GUARD
  if (pid() == 0) {
    FILE *fp = fopen("boundary_guard.tsv", i == 0 ? "w" : "a");
    if (!fp) { perror("boundary_guard.tsv"); exit(1); }
    if (i == 0) fprintf(fp,"i\tt_star\tS0_t_star\tmax_edge_height_departure\tthreshold\n");
    fprintf(fp,"%d\t%.16e\t%.16e\t%.16e\t%.16e\n",
            i,t,SLOPE_TAN*t,edge_departure,(double)CASE_FR_GUARD_HEIGHT);
    fclose(fp);
  }
#endif
#if CASE_FR_STOP_BEFORE_WRAP
  if (edge_departure > CASE_FR_GUARD_HEIGHT) {
    fprintf(ferr,"# STOP boundary guard: edge height departure %g > %g at t*=%g. "
                 "Increase Lx or inspect base-state drift; this is not periodic-wave data.\n",
            edge_departure,(double)CASE_FR_GUARD_HEIGHT,t);
    return 1;
  }
#endif
}
#endif // enabled boundary stopping or explicitly requested guard output
...
event diagnostics (i += CASE_DIAGNOSTICS_EVERY)
{
  ...
  foreach (reduction(+:leaves) reduction(min:levmin) reduction(max:levmax)
           ...
           reduction(max:far_air_max_level)
    ...
    if (y + 0.5*Delta > near_air_y*CEILING_FINE_FRACTION &&
        cm[] > INTERFACE_EPS)
      far_air_max_level = max(far_air_max_level, level);
    const double vol = dv();
  ...
  fprintf (ferr,
           ...
           lower_min_level, interface_min_level, far_air_max_level,
...
{
  scalar * fields = {f_lower, f, f1, f2, f3, u.x, u.y, p,
                     gammaDot, muMix, rhov, level_field};
  const int nfields = 12;
  const double ds = finest_delta();
  int nsamples = 0;
  for (double yy = ds/2.; yy < near_air_y - ds/4.; yy += ds)
    nsamples++;
  coord * points = (coord *) malloc (nsamples*sizeof(coord));
  double * values = (double *) malloc ((size_t)nsamples*nfields*sizeof(double));
  if (!points || !values)
    case_fatal ("cannot allocate vertical-slice buffers");
  int j = 0;
  for (double yy = ds/2.; yy < near_air_y - ds/4.; yy += ds)
    points[j++] = (coord){xx, yy, 0.};

  /* These operations MUST be called on EVERY rank. In particular, do not put
     interpolate_array() inside the pid()==0 branch: it performs reductions. */
  boundary (fields);
  interpolate_array (fields, points, nsamples, values, true);

  /* Only root opens/truncates/writes the shared output file. */
  if (pid() == 0) {
    FILE * fp = fopen (name, "w");
    if (!fp)
      case_fatal ("cannot open vertical-slice output file");
    fprintf (fp, "z_star\tfLower\tfLiquid\tf1\tf2\tf3\tu_x_star\tu_z_star"
                 "\tp_star\tgamma_star\teta_star\tr_star\tlevel\tcs\n");
    for (int row = 0; row < nsamples; row++) {
      fprintf (fp, "%.12e", points[row].y);
      for (int col = 0; col < nfields; col++)
        fprintf (fp, "\t%.12e", values[(size_t)row*nfields + col]);
      /* Keep the old slice-table layout: cs=1 means all cells are fluid.
         It is an output literal, not an allocated geometry field. */
      fputs ("\t1.000000000000e+00\n", fp);
    }
    if (fclose (fp) != 0)
      case_fatal ("cannot close vertical-slice output file");
  }
  free (points);
  free (values);
}
                                    ...
                                    double time)
{
  ...
  if (!fp)
    case_fatal ("cannot open per-rank interface output");
  output_facets (c, fp);
  fclose (fp);
#if _MPI
  MPI_Barrier (MPI_COMM_WORLD);
...
event output_gfs_files (t = 0.; t += GFS_DT; t <= END_TIME + 1.e-12)
{
  ...
  scalar * gfs_fields = {f, f_lower, f1, f2, f3,
...
#endif // !CASE_DISABLE_FIELD_OUTPUTS

event stop (t = END_TIME)
{
  case_write_summary ("end-time");
  if (wave_amplitude_fp) {
