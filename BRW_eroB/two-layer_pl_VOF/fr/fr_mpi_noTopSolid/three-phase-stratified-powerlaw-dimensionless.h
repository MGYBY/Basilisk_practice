...
field `mu` stores dimensionless *dynamic* apparent viscosity. The intended
continuum momentum equation is

    r Du/Dt = -grad(p) + div(2 eta D) + r a.

This non-embedded v2p7 case uses upstream viscosity.h for the complete
symmetric-stress operator. The parent v2p6 top-solid case selected viscosity-embed.h;
its componentwise diffusion is not generally equivalent when viscosity varies.
Rheological formulas and MPI property synchronization in this header are unchanged.
...
/* Standalone inclusion retains the old detailed accounting; the production
   case selects zero in case_runtime_options.h before including this header. */
#ifndef STRATIFIED_VOLUME_AUDIT
# define STRATIFIED_VOLUME_AUDIT 1
#endif
...
static void stratified_enforce_hierarchy (void)
{
  ...
  foreach (reduction(max:max_h) reduction(max:max_b)
           ...
           reduction(+:corrected))
#else
  /* Keep the two topology maxima in the correction loop. Only the four
     before/after volume sums and correction counter are diagnostic-only. */
  foreach (reduction(max:max_h) reduction(max:max_b))
#endif
  {
...
event properties (i++)
{
  /* Explicit dependencies for MPI/coarse-fine halo values. This is a
     defensive synchronization; recent qcc versions also insert automatic BCs. */
  boundary ({f_lower, f});
  boundary ((scalar *){u});
  ...
  /* Keep the alias outside the loop and explicitly synchronize the complete
     vector afterwards (not a single component). */
  face vector muv = mu;
  foreach_face() {
  ...
  /* Coefficients must be valid on the multigrid hierarchy as well as leaves.
     Native viscosity()/poisson() also restrict them; no leaf formula changes. */
  restriction ((scalar *){alphav, muv, rhov});
}
