...
optional phase-selective streamwise gravity, a selectable localized constant-Froude or legacy divergence-free velocity
...
event init (i = 0)
{
  ...
  fr_init(&front_runner_ic, &profile_u0,
          ...
          CASE_FR_WAVELENGTH, CASE_FR_CENTER, LIQUID_DEPTH, ceiling_y);
  fprintf(ferr, "# front-runner: support=[%g,%g] center=%g wavelength=%g "
          ...
          front_runner_ic.q0, front_runner_ic.qt);
#endif
...
event front_runner_boundary_guard (i += 10)
{
#if CASE_FRONT_RUNNER_ENABLED
  double edge_departure = 0.;
  foreach(reduction(max:edge_departure)) {
    if (cs[] > 0. && (x < CASE_FR_GUARD_WIDTH || x > LX-CASE_FR_GUARD_WIDTH)) {
...
static void write_vertical_slice (const char * name, double xx)
{
  ...
  fprintf (fp,
           "z_star\tfLower\tfLiquid\tf1\tf2\tf3\tu_x_star\tu_z_star\tp_star\tgamma_star\teta_star\tr_star\tlevel\tcs\n");
