...
#define R0 1.0
#define disAmp 0.10
#define distPeriod 32.0

#define DOMAINLENGTH 600.0
...
double distAreaInlet(double t)
{
  //   "type-c" inlet dist
  // if (t<=distPeriod/2.0)
    return (pi*R0*R0*(1.0 + disAmp * sin(2. * pi * t / distPeriod)));
  // else
    // return normalDepth;
}
...
double distQInlet(double t)
{
  ...
  // if (t<=distPeriod/2.0)
    return (Q0*(1.0 + disAmp * sin(2. * pi * t / distPeriod)));
  // else
...
event init (i = 0) {
  ...
  // "wavemaker" inlet BC
  a[left] = dirichlet(distAreaInlet(t));
